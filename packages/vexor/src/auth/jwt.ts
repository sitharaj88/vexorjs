...
function base64UrlEncode(data: Uint8Array | string): string {
  ...
  // Use chunked approach to avoid exceeding max call stack with spread operator
  let binary = '';
...
export class JWT {
  ...
  private async verifyRsa(data: string, signature: string): Promise<boolean> {
    if (!this.options.publicKey) {
      throw new Error('Public key is required for RSA verification. Private keys cannot be used for verification.');
    }

    const { name, hash } = getRsaAlgorithm(this.options.algorithm);

    const keyData = this.pemToArrayBuffer(this.options.publicKey, 'PUBLIC');
