...
/**
 * Validate IPv6 address (supports compressed forms like ::1, fe80::1)
 */
function isValidIPv6(value: string): boolean {
  ...
  if (value.includes('.')) {
    ...
    const ipv4Part = value.substring(lastColon + 1);
  ...
  if (doubleColonParts.length === 2) {
    ...
    return allGroups.every(g => /^[0-9a-f]{1,4}$/i.test(g));
  }
...
function validateArray<T extends TSchema>(
  ...
  return ctx.issues.length === issuesBefore;
}
