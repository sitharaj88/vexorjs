...
export function createFactory<T extends Record<string, unknown>>(
  ...
  const factory: Factory<T> = {
    ...
    withState(name: string): Factory<T> {
      // Create a new factory with the state baked into the definition
      const allStates = { ...definition.states, ...states };
      const previousStates = [...activeStates, name];
      const originalDef = definition.definition;
      const newDefinition: FactoryDefinition<T> = {
        ...definition,
        states: allStates,
        definition: () => {
          let attrs = originalDef();
          for (const stateName of previousStates) {
            const stateAttrs = allStates[stateName];
            if (stateAttrs) {
              const resolved = typeof stateAttrs === 'function' ? stateAttrs() : stateAttrs;
      ...
      return createFactory(db, newDefinition);
    },
  };
