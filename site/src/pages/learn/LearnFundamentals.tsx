...
// 405 Method Not Allowed - Path exists, but not for this method
...
// 4. Handle POST requests with a body
...
//     |
//     v
// [2] Hook chain executes (onRequest hooks / middleware)
//     |
//     v
// [3] Route matching (radix tree lookup)
//     |
//     v
// [4] Schema validation (if a schema is defined)
//     |
//     v
...
app.get('/users/:id', async (ctx) => {
  // ctx.method                  -> 'GET'
  // ctx.path                    -> '/users/42'
  // ctx.params                  -> { id: '42' }  (typed from the path)
  // ctx.query                   -> { include: 'posts' }  (from ?include=posts)
  // ctx.header('content-type')  -> read a single request header
  // ctx.headers                 -> the full Headers object
...
// CREATE a todo
// The bare schema shorthand: { body } is equivalent to { schema: { body } }
app.post('/api/todos', {
  body: Type.Object({
  ...
  // ctx.body() is typed from the schema above
...
const chapters = [
  ...
  { num: '06', title: 'RESTful API Design', desc: 'Build a complete CRUD API', color: 'from-primary-400 to-primary-600' },
];
...
export default function LearnFundamentals() {
  return (
    <div className="space-y-12">
      {/* Header */}
      <div>
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-full text-sm font-medium mb-4">
          <span className="w-2 h-2 bg-emerald-500 rounded-full"></span>
          Beginner
        </div>
        <h1 id="fundamentals" className="text-4xl font-bold text-slate-900 dark:text-white mb-4">
          Fundamentals
        </h1>
      ...
      <section id="http-basics">
        ...
        <InfoBlock variant="tip" title="Key Takeaway">
          Think of your API as a set of resources.
          Use <code className="prose-code">GET /users</code> to list users,{' '}
          <code className="prose-code">POST /users</code> to create one, and{' '}
          <code className="prose-code">GET /users/:id</code> to fetch a specific user.
          Vexor handles the HTTP plumbing for you: it answers with an automatic{' '}
          <code className="prose-code">405 Method Not Allowed</code> (including the{' '}
          <code className="prose-code">Allow</code> header) when a path exists under other methods,
          and serves <code className="prose-code">HEAD</code> requests from your GET handlers per
          the HTTP spec.
        </InfoBlock>
      </section>
      ...
      <section id="first-api">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-4">
          Let's build a working API. You'll create routes, handle parameters, and respond with JSON -- all in
          under 20 lines of code. Note how the request body is read with{' '}
          <code className="prose-code">await ctx.body()</code>: body parsing is asynchronous, and when a route
          declares a schema the parsed body is fully typed.
        </p>
        ...
        <div className="mb-6 p-4 bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl">
          ...
          <div className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
            ...
            <p>2. Run: <code className="prose-code">npx @vexorjs/cli new my-first-api</code></p>
            <p>3. Navigate: <code className="prose-code">cd my-first-api</code></p>
          </div>
        </div>

        <CodeBlock code={firstApiCode} filename="src/index.ts" showLineNumbers />

        <InfoBlock variant="tip" title="Try it out">
          Run your server with <code className="prose-code">npm run dev</code>, then visit{' '}
          <code className="prose-code">http://localhost:3000</code> in your browser
          or use <code className="prose-code">curl http://localhost:3000/greet/Vexor</code>.
        </InfoBlock>
      </section>
      ...
      <section id="request-response">
        ...
        <p className="text-slate-600 dark:text-slate-400 mb-4">
          ...
          know where to add logic, validation, and error handling. Note that{' '}
          <code className="prose-code">ctx.headers</code> is a standard{' '}
          <code className="prose-code">Headers</code> object; use{' '}
          <code className="prose-code">ctx.header('name')</code> as a convenient shortcut for reading a
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm">
            {['Receive', 'Hooks', 'Route Match', 'Validate', 'Handle', 'Respond'].map((step, i) => (
              <span key={step}>
                {i > 0 && <span className="text-slate-400 mr-2">&rarr;</span>}
                <span className={`px-3 py-1 rounded-full ${i === 4 ? 'bg-primary-500/20 text-primary-600 dark:text-primary-400 font-medium' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                  {step}
                </span>
              </span>
            ))}
      ...
      <section id="restful-crud">
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">
          <span className="text-primary-500 mr-2">06</span> Building a RESTful CRUD API
        </h2>
        <p className="text-slate-600 dark:text-slate-400 mb-4">
          Let's put it all together. Here's a complete Todo API with Create, Read, Update, and Delete operations,
          including input validation using Vexor's schema system. You can pass the schema directly as the second
          argument (<code className="prose-code">{'{ body }'}</code>) or nest it under{' '}
          <code className="prose-code">{'{ schema: { body, query, params } }'}</code> -- both forms are valid.
        </p>
        <CodeBlock code={restfulApiCode} filename="todo-api.ts" showLineNumbers />

        <InfoBlock variant="info" title="Validation behavior">
          Schema properties are <strong>required by default</strong> -- wrap a property in{' '}
          <code className="prose-code">Type.Optional(...)</code> to make it optional. When validation
          fails, Vexor responds with <code className="prose-code">400 Bad Request</code> and a JSON body
          of the shape{' '}
          <code className="prose-code">{"{ error, code: 'VALIDATION_ERROR', issues }"}</code>, where{' '}
          <code className="prose-code">issues</code> lists each failing path and message. See the{' '}
          <Link to="/validation" className="underline">Validation guide</Link> for details.
        </InfoBlock>
      ...
      <section className="card bg-slate-50 dark:bg-slate-800/50">
        ...
        <div className="flex flex-wrap gap-3">
          <Link to="/learn/building-apis" className="btn-primary">
            Continue to Building APIs <ArrowRight className="w-4 h-4 ml-2" />
          </Link>
